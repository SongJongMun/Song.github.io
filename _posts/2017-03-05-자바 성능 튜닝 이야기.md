...
### Mark-sweep-compact Collection(Major GC)
...
초기 표시 : 매우 짧은 대기 시간으로 `살아 있는 개체`를 `찾는다`(`1개 CPU`)

컨커런트 표시 : 서버 수행과 동시에 살아있는 객체에 표시를 해놓는다(`동시수행`)

재표시 : 이전 수행에서 표시하는 `동안` `변경된 객체`에 대해 `다시` 표기한다.

컨커런트 스윕 : 표시되어 있는 쓰레기를 정리한다.
...
초기 표시 단계(STW) : Old 영역에 있는 객체에서 `Survivor영역`의 객체를 `참조`하고 있는 객체들을 `표시`

기본 구역 스캔 : Old 영역 참조를 위해 Survior 영역 탐색

컨커런트 표시 : 전체 힙 영역에서 살아있는 객체를 찾는다.

재 표시 단계(STW) : 힙에 살아있는 객체들의 표시 작업을 완료한다.

청소단계(STW) : 살아있는 객체와 비어있는 구역을  식별하고, 필요없는 개체들을 지움, 비어있는 구역 초기화

복사단계(STW) : 살아있는 객체들을 비어있는 구역으로 모은다.
...
ListAddTest.testArrayListAdd             avgt  200   0.106 ±  0.001  ms/op
ListAddTest.testLinkedListAdd            avgt  200   0.144 ±  0.001  ms/op
ListAddTest.testVectorAdd                avgt  200   0.099 ±  0.001  ms/op
--------------------------------------------------------------------------
...
List Get의 경우 LinkedList가 가장 느리며 나머지는 동일하다.
List Remove의 경우 Last Remove는 성능이 동일하며, First Remove의 경우 ArrayList와 Vector가 가장 느리다.
---
# JMH 를 사용하여 Java SE 에 있는 Map을 구현한 클래스들의 추가/조회/삭제 기능의 성능을 비교하시오.

```
Benchmark                                Mode  Cnt  Score   Error  Units
MapAddTest.addSeqHashMap                 avgt  200  0.569 ± 0.019  ms/op
MapAddTest.addSeqHashtable               avgt  200  0.730 ± 0.029  ms/op
MapAddTest.addSeqLinkedHashMap           avgt  200  0.594 ± 0.022  ms/op
MapAddTest.addSeqTreeMap                 avgt  200  1.412 ± 0.044  ms/op
--------------------------------------------------------------------------
...
Map Put의 경우 TreeMap가 가장 느리다.
Map Get의 경우 TreeMap, HastTable순으로의 느리다.
Map Remove의 경우 HashTable이 가장 느리며 나머지는 비슷하다.
